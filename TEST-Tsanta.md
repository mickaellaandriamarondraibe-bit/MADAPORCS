...
| REP-04 | Ouvrir le detail d'un groupe                       | Informations completes (lots, dates, statut)                   |     ok   |       affiche une petite incoherence de valeur de champs (corriger)          |
...
| ALE-03 | Marquer une alerte comme traitee    | L'alerte passe en "traitee" et sort des alertes actives |    ok    |   on entre juste dans une page mais il y a pas vraiment une traitement normalement on devrait traiter en ajoutant une boutton         |
...
| IMP-03 | Importer un fichier Excel valide (base sur le modele) | Donnees importees, message de succes                |   OK    |    le modele vaccination ne marche pas ca affiche toujours 0 lignes importée or il y a quelque chose dedans            |
| IMP-04 | Importer un fichier au mauvais format                 | Erreur claire, aucune donnee corrompue              |  ok      |    seule les fichier csv peut etre importer donc quand c'est mauvaise format ca s'affiche pas      |
| IMP-05 | Importer un fichier avec lignes invalides             | Rejet/rapport des lignes en erreur, pas de plantage |   ok     |  l'importation marche mais dans l'affichage de liste sur le ligne manquant il y a rien         |
| IMP-06 | Exporter en Excel                                     | Fichier Excel telecharge et ouvrable                |   ok     |  ca ne telecharge pas vraiment          |
...
| IMP-09 | ajout de meme id dans le model de lots        | une erreur                      |    ok    |           |
