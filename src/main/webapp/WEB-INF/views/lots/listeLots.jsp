...
<div class="page-head">
  <div><h1>Lots de porcs</h1><p>Création, suivi et archivage des lots</p></div>
  <a class="btn btn--primary" href="${ctx}/lots/form"><i class="fa-solid fa-plus"></i> Nouveau lot</a>
</div>

<%-- Filtres -> GET /lots, bind LotFiltreDTO --%>
<form method="get" action="${ctx}/lots" class="toolbar">
  <div class="search"><input class="input" type="text" name="codeLot" value="${filtre.codeLot}" placeholder="Code du lot…"></div>
...
<div class="card">
  <div class="table-wrap">
    <table class="tbl">
      <thead>
        <tr><th>Code</th><th>Sexe</th><th>Race</th><th>Objectif</th><th class="num">Effectif</th><th>Statut</th><th class="actions">Actions</th></tr>
      </thead>
      <tbody>
        <c:choose>
          <c:when test="${not empty lots}">
            <c:forEach var="l" items="${lots}">
              <tr>
                <td><a href="${ctx}/lots/${l.id}"><b>${l.codeLot}</b></a></td>
                <td>
                  <c:choose>
                ...
                <td>${l.race}</td>
                <td>${l.objectif}</td>
                <td class="num"><b>${l.effectifActuel}</b> <span class="muted">/ ${l.effectifInitial}</span></td>
                <td>
                  <c:choose>
                    <c:when test="${l.statut == 'ACTIF'}"><span class="badge badge--green"><span class="dot"></span>Actif</span></c:when>
                    <c:otherwise><span class="badge badge--gray">Archivé</span></c:otherwise>
                  </c:choose>
                </td>
                <td class="actions">
          ...
          <c:otherwise>
            <tr><td colspan="7"><div class="empty"><i class="ico fa-solid fa-piggy-bank"></i><p>Aucun lot ne correspond.</p><a class="btn btn--primary" href="${ctx}/lots/form">Créer un lot</a></div></td></tr>
          </c:otherwise>
        </c:choose>
      </tbody>
    </table>
